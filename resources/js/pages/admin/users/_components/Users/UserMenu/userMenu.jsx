...
import Form, {AsyncAutoComplete, DateTimePicker} from "@/components/Form";
...
const UserMenu = ({user}) => {
    ...
    const menuItems = useMemo(() => {
        ...
        if (user.manage_policy && auth.can("assign:roles")) {
            items.push(
...
const AssignRole = ({closeModal, user}) => {
    ...
    const submitForm = useCallback(
        (values) => {
            const url = route("admin.role.assign", {user: user.id});
    ...
    const optionsFetcher = async (value) => {
        const url = route("admin.roles.paginate");

        const {data: paginated} = await request.get(url, {
            params: {search: {name: value}}
        });

        return map(paginated.data, "name");
    };
        ...
        <Form form={form} onFinish={submitForm}>
            <ModalContent>
                <Form.Item
                    ...
                    initialValue={user.all_roles}>
                    <AsyncAutoComplete multiple fetcher={optionsFetcher} />
                </Form.Item>
            </ModalContent>
